...
function Home() {
	return (
		<motion.div initial='initial' whileInView='animate' className='px-2 py-20 max-w-7xl mx-auto overflow-x-hidden md:overflow-x-visible'>
			<Sidebar />
			<Landing />
			<Customers />
			<Steps />
	...
	return (
		<div className='flex gap-10 flex-col items-center xl:flex-row text-center xl:text-left'>
			<div className='flex flex-col items-center xl:items-start justify-center gap-8 lg:max-w-2xl'>
				<motion.h1 variants={slideIn("down", 0.5)} className='lg:text-6xl text-3xl font-semibold font-serif' style={{ lineHeight: "1.2" }}>
					Free Online Resume Builder: Make Yours in Minutes
				</motion.h1>

				<motion.p variants={slideIn("down", 0.5, 0.3)} className='md:text-xl'>
				...
				<Button link='/resume' text='Build a resume' />
			</div>

			<motion.div variants={slideIn("right", 0.5)}>
				<img src={image} className='w-[90%] mx-auto' alt='' />
			</motion.div>
		</div>
	);
};
